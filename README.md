# 王梦珂 Mengke Wang

好事发生 App 开发者｜好事引力创始人

把复杂经验和判断，翻译成普通人可以直接使用、验证和分享的工具。

---

## Mengke Skills

我在持续发布一套 AI 工具，分两条线：

### 好事引力 · 产品与品牌增长

从决定做什么，到让好产品被对的人看见。

| 工具 | 解决什么问题 |
| --- | --- |
| [Should I Build? · 五书会诊台](https://github.com/mengke-wang/should-i-build) | 用五本书判断产品或功能应该 BUILD、TEST、PARK 还是 KILL |
| [小红书运营手册 · AI 工作台](https://github.com/mengke-wang/xiaohongshu-ai-workbench) | 主页、标题、选题、栏目、评论区和成交路径 |
| [《把才华变成钱》Skill Pack](https://github.com/mengke-wang/talent-money-skills) | 把能力、证据、内容和服务整理成可被购买的第一版 |
| [中文去 AI 味与文采增强](https://github.com/mengke-wang/zh-humanizer-literary) | 去掉模板感和营销腔，保留事实、判断和作者声音 |
| [微信朋友圈防折叠改写](https://github.com/mengke-wang/wechat-anti-fold) | 双号发同一条朋友圈，改写出不被折叠的变体版本 |

### 好事发生 · 人生设计

让你看见自己、做出选择，把生活设计得更好。

| 工具 | 解决什么问题 |
| --- | --- |
| [徐子平 · AI 八字排盘与批命](https://github.com/mengke-wang/xuziping-bazi) | 先用确定性程序排盘，再根据盘面做传统文化解读 |
| [人类图 · 自我实验读图](https://github.com/mengke-wang/human-design-reader) | 排图、核对时间敏感性，把盘面翻译成现实中的自我观察 |

完整目录和体系地图见 [mengke-skills](https://github.com/mengke-wang/mengke-skills)。

---

- [好事发生 Good Happens](https://goodhappens.app/)
- [好事引力 Haoshi Yinli](https://haoshiyinli.com/)
